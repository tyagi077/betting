...
function WinningPrice(){ {
    ...
    const fetchData = async () => {
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/winninggame/fetch.php`)
            .then(res => {
                setData(res.data);
            })
    ...
    const deleteData = async (id) => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/winninggame/delete.php?deleteid=${id}`);
    ...
    return (
        <div className='flex flex-col gap-4 text-white'>
            ...
            <Link to={`/add/winning`}><button className='btn btn1'><i className="fa-solid fa-plus"></i> Add</button></Link>
                    ...
                    <table className="data-table" id="dataTable">
                        <thead>
                            <tr>
                                ...
                                <th onClick={() => sortTable(1)}>Calculation Type</th>
                        ...
                        <tbody id="tableBody">
                            {
                                ...
                                newData?.splice(startIndex, endIndex).map((obj, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{obj.category}</td>
                                        <td>{obj.type}</td>
                                        <td>{obj.calculation}</td>
                                        <td>{obj.value}</td>
                                        <td className="">
                                            <div className="flex gap-3 items-center">
                                            <Link to={`/update/winning/${obj.id}`}><i className="updateFormBtn cursor-pointer hover:text-white text-[#dcdada] fa-regular fa-pen-to-square bg-[#406793] p-2 rounded-lg "></i></Link>
