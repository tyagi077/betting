...
function Slot() {
    ...
    const fetchData = async () => {
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/manage/display.php`)
            .then(res => {
    ...
    return (
        <div className='flex flex-col gap-4 text-white'>
            ...
            <div className="bg-[#0e1726] py-3 px-5 rounded-lg ">
                ...
                <Link to={`/add/manage`}><button className='btn btn1'><i className="fa-solid fa-plus"></i> Add</button></Link>
                    ...
                    <table className="data-table" id="dataTable">
                        <thead>
                            <tr>
                                ...
                                <th onClick={() => sortTable(1)}>Status
                                </th>
                                <th onClick={() => sortTable(2)}>image
                                </th>
                                <th onClick={() => sortTable(3)}>name
                                </th>
                                <th onClick={() => sortTable(4)}>category name
                                </th>
                                <th onClick={() => sortTable(5)}>slot
                                </th>
                        ...
                        <tbody id="tableBody">
                            {
                                ...
                                newData?.splice(startIndex, endIndex).map((obj, index) => (
                                    <tr key={index}>
                                        <td>{obj.id}</td>
                                        <td>{obj.status}</td>
                                        <td><img src={obj.image} className='w-[8rem] h-[5rem] object-cover ' alt="" /></td>
                                        <td>{obj.name}</td>
                                        <td>{obj.category}</td>
                                        <td>{obj.time_start} - {obj.time_end}</td>
                                        <td className="">
                                            <div className="flex gap-3 items-center">
                                                <Link to={`/update/manage/${obj.id}`}><i className="updateFormBtn cursor-pointer hover:text-white text-[#dcdada] fa-regular fa-pen-to-square bg-[#406793] p-2 rounded-lg "></i></Link>
