...
function GameName() {
    ...
    const handleFilter = () => {
        ...
        if (inputValue) {
            let filtered = data.filter(news =>
                news.ename.toUpperCase().includes(inputValue.toUpperCase())
            );
    ...
    const deleteData = (id) => {
        try {
            axios.get(`${import.meta.env.VITE_BACKEND_URL}/game/delete.php?deleteid=${id}`)
    ...
    return (
        <div className='flex flex-col gap-4 text-white'>
            ...
            <Link to={`/add/game`}><button className='btn btn1'><i className="fa-solid fa-plus"></i> Add</button></Link>
                        ...
                        <tbody id="tableBody">
                            {
                                ...
                                newData?.splice(startIndex, endIndex).map((obj, index) => (
                                    <tr key={index}>
                                        <td>{obj.id}</td>
                                        <td><img src={obj.image} alt="" className='w-[8rem] h-[4rem]' /></td>
                                        <td>{obj.ename}</td>
                                        <td>{obj.label}</td>
                                        <td>{obj.status}</td>
                                        <td className="">
                                            <div className="flex gap-3 items-center">
                                            <Link to={`/update/game/${obj.id}`}><i className="updateFormBtn cursor-pointer hover:text-white text-[#dcdada] fa-regular fa-pen-to-square bg-[#406793] p-2 rounded-lg "></i></Link>
