...
function Slider() {
    ...
    const fetchData = async () => {
        axios.get(`${import.meta.env.VITE_BACKEND_URL}/slider/display.php`)
            .then(res => {
                setData(res.data);
            })
    ...
    const handleFilter = () => {
        ...
        if (inputValue) {
            let filtered = data.filter(news =>
                news.title.toUpperCase().includes(inputValue.toUpperCase())
            );
    ...
    const deleteData = async (id) => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/slider/delete.php?deleteid=${id}`);
    ...
    return (
        <div className='flex flex-col gap-4 text-white'>
            ...
            <Link to={`/add/slider`}><button className='btn btn1'><i className="fa-solid fa-plus"></i> Add</button></Link>
                        ...
                        <tbody id="tableBody">
                            {
                                ...
                                newData?.splice(startIndex, endIndex).map((obj, index) => (
                                    <tr key={index}>
                                        <td>{obj.id}</td>
                                        <td><img src={obj.link} alt="" className='w-[8rem] h-[4rem]' /></td>
                                        <td>{obj.title}</td>
                                        <td>{obj.status}</td>
                                        <td className="">
                                            <div className="flex gap-3 items-center">
                                            <Link to={`/update/slider/${obj.id}`}><i className="updateFormBtn cursor-pointer hover:text-white text-[#dcdada] fa-regular fa-pen-to-square bg-[#406793] p-2 rounded-lg "></i></Link>
